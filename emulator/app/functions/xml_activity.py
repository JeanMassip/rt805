...
def build_new_bar_xml(name, lon, lat):
    create_bar = ET.Element("Bar")
    ET.SubElement(create_bar, "Name").text = name
    ET.SubElement(create_bar, "Position_lon").text = str(lon)
    ET.SubElement(create_bar, "Position_lat").text = str(lat)
    tree = ET.ElementTree(create_bar)
    return ET.dump(tree)
